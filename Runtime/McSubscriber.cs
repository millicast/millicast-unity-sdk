...
namespace Dolby.Millicast
{
  ...
  public class McSubscriber : MonoBehaviour
  {
    ...
    private List<RTCRtpReceiver> _rtpReceivers = new List<RTCRtpReceiver>();
    private VideoConfigurator _videoConfigurator;
    private AudioConfigurator _audioConfigurator;
    ...
    /// <summary>
    /// Delegate called when the there is an available video frame to transform.
    ///</summary>
    ...
    private void EstablishPeerConnection(ref RTCIceServer[] iceServers)
    {
      ...
      _pc.OnTrack += (e) =>
      {
        _rtpReceivers.Add(e.Receiver);
        if (e.Track is VideoStreamTrack track)
        {
          // Setup Frame Transformers if available.
          _videoConfigurator = new VideoConfigurator(new ReceiveFrameTransformer(e.Receiver));
          if (_onTransformableVideoFrame != null) {
            _videoConfigurator.SetTransform(_onTransformableVideoFrame);
          }
        ...
        {
          // Setup Frame Transformers if available.
          _audioConfigurator = new AudioConfigurator(new ReceiveFrameTransformer(e.Receiver));
          if (_onTransformableAudioFrame != null) {
            _audioConfigurator.SetTransform(_onTransformableAudioFrame);
          }
    ...
    /// <summary>
    /// Set the transform for incoming video. This callback will receive encoded frames that can
    /// be transformed before being decoded. 
    /// </summary>
    /// <param name="transformDelegate">The delegate to be called on every encoded frame</param>
    public void SetVideoTransform(FrameMetadata.DelegateOnTransformableVideoFrame transformDelegate) {
      _onTransformableVideoFrame = transformDelegate;
      if (isSubscribing && _videoConfigurator != null) {
        _videoConfigurator.SetTransform(transformDelegate);
      }
    }

    /// <summary>
    /// Set the transform for incoming audio. This callback will receive encoded frames that can
    /// be transformed before being decoded.
    /// </summary>
    /// <param name="transformDelegate">The delegate to be called on every encoded frame</param>
    public void SetAudioTransform(FrameMetadata.DelegateOnTransformableAudioFrame transformDelegate) {
      _onTransformableAudioFrame = transformDelegate;
      if (isSubscribing && _audioConfigurator != null) {
        _audioConfigurator.SetTransform(transformDelegate);
      }
    }
